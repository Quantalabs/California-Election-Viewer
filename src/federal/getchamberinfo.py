...
class house:
    ...
    def representatives(self):
        ...
        for member in data['members']:
            if member['state'] == 'CA':
                careps.append({'first_name': member['first_name'],
                              'last_name': member['last_name'],
                               'district': member['district'], 'id': member['id']})
            else:
                continue
    ...
    def representative(self, district):
        url = f"https://api.propublica.org/congress/v1/members/house/CA/{district}/current.json"
        headers = {'X-API-Key': self.propublica}
        r = requests.get(url, headers=headers)
        data = r.json()['results'][0]
...
class senate:
    ...
    def senators(self):
        ...
        for member in data['members']:
            if member['state'] == 'CA':
                senators.append({'first_name': member['first_name'],
                                 'last_name': member['last_name'], 'id': member['id']})
            else:
                continue
    ...
    def senator(self, id):
        url = f"https://api.propublica.org/congress/v1/members/senate/CA/current.json"
        headers = {'X-API-Key': self.propublica}
        r = requests.get(url, headers=headers)
        data = r.json()['results']
